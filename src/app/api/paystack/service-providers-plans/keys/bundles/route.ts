...
export async function GET(request: NextRequest) {
  try {
    ...
    // Filter out duplicates by transactionId - keep only the first occurrence
    const uniqueBundles = bundles.filter(
      (bundle, index, self) =>
        index ===
        self.findIndex((b) => b.transactionId === bundle.transactionId)
    );

    return NextResponse.json({ success: true, bundles: uniqueBundles });
  } catch (error) {
